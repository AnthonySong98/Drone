...
def jsonmake(header,type,lat=None,lon=None,battery=None,mode=None,status=None,version=None,msg=None):
    return json.dumps([{'Header':header,'type':type},
                       {'lat':lat,'lon':lon,'version':str(version),'battery':battery,
...
def goto(dNorth, dEast):
    ...
    while vehicle.mode.name == "GUIDED":  # Stop action if we are no longer in guided mode.
        ...
        send_queue.put(jsonmake('RE', TYPE, vehicle.location.global_relative_frame.lat,
                                ...
                                vehicle.battery.voltage, vehicle.mode.name,
