...
class MyObject(QObject):
    ...
    def setval(self):
        pass
    ppval=pyqtProperty(list,readval,setval)
...
#        self.thread2() #启动监听线程
    ...
    def initUI(self):
        mainlayout = QHBoxLayout()  # 主页面
        # 这个是四台无人机状态的控件
#        textedit1 = QListWidget()
        self.textedit1 = QListWidget()
        self.textedit5 = QListWidget()
        self.brower=QWebEngineView()
        self.textedit2 = QListWidget()
        self.textedit3 = QListWidget()
        self.textedit4 = QListWidget()
        self.textedit1.setMaximumSize(QSize(200, 150))
        self.textedit2.setMaximumSize(QSize(200, 150))
        self.textedit3.setMaximumSize(QSize(200, 150))
        self.textedit4.setMaximumSize(QSize(200, 150))
        glayout = QGridLayout()  # pix的布局
        ...
        pixwidget = QWidget()  # 左边的
        pixwidget.setFixedSize(QSize(400, 300))
        pixwidget.setLayout(glayout)  # 这个是四台无人机状态的控件
        # 这个是连接按钮和指令台的控件。
        btn = QPushButton('连接无人机')
        self.textedit5.setFixedSize(QSize(250, 150))
        btn.setFixedSize(QSize(100, 100))  # 改变按钮的大小
        hlayout = QHBoxLayout()
        hlayout.addWidget(btn)
        hlayout.addWidget(self.textedit5)
        connectwidget = QWidget()
        connectwidget.setFixedSize(QSize(400, 150))
        connectwidget.setLayout(hlayout)  # 这个是连接按钮和指令台的控件。
        # 三个模式的控件
        modewidget = QWidget()
        self.modebtn1 = QPushButton('导航模式')
        self.modebtn2 = QPushButton('RC模式')
        self.modebtn3 = QPushButton('--模式')
        modelayout = QHBoxLayout()
        self.modebtn1.setFixedSize(QSize(100, 100))
        self.modebtn2.setFixedSize(QSize(100, 100))
        self.modebtn3.setFixedSize(QSize(100, 100))
        #刚开始是不可以活动的
        ...
        modewidget.setFixedSize(QSize(400, 100))
        ...
        #       pixwidget.setGeometry(QRect(10, 60, 100, 60))
        ...
        #创建一个侨界对象
        channel=QWebChannel()
        map.setFixedSize(QSize(600, 550))
        ll=QVBoxLayout()
        map.setLayout(ll)
        url = r'E:/python3/PyQtT/index.html'
        self.brower.load(QUrl(url))
        ll.addWidget(self.brower)
#        map.setCentralWidget(self.brower)
        # 主页面的修改
    ...
    def workthread(self):
        #这个线程是为了接收。
        # t = threading.Thread(target=self.monitorPC,args='1')
        # t.start()
        # t = threading.Thread(target=self.monitorPC, args='2')
        # t.start()
        # t = threading.Thread(target=self.monitorPC, args='3')
        # t.start()
        # t = threading.Thread(target=self.monitorPC, args='4')
        # t.start()
        #这个线程是为了处理消息
    ...
    #                 self.textedit1.addItem('lon:' + str(msg[1]['lon']))
    #             if(msg[0]['Header'] == 'CMD'):
    #                 self.textedit5.addItem(msg[1]['msg'])
    #                 #每次都是到最底下。
    #                 self.textedit5.setCurrentRow(self.textedit5.count()-1)
    #         time.sleep(0.5)
