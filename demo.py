...
def display():
    x=vehicle.location.global_relative_frame.lon
    y=vehicle.location.global_relative_frame.lat
    altitude_show=vehicle.location.global_relative_frame.alt
    airspeed_show=vehicle.airspeed
    voltage_show=vehicle.battery.voltage        
    global i
    i+=1
    t.append(i)
    altitude_data.append(altitude_show)
    airspeed_data.append(airspeed_show)
    voltage_data.append(voltage_show)
    plt.subplot(221)
    plt.axis([149.163,149.166,-35.362,-35.364])
    ax = plt.gca() 
    ax.xaxis.get_major_formatter().set_powerlimits((-8,6))
    ax.yaxis.get_major_formatter().set_powerlimits((-8,6))
    plt.scatter(x, y,c='r',s=3)
    plt.title('Coordinates')  
    plt.xlabel('longitude')  
    plt.ylabel('latitude')
    plt.subplot(222)
    plt.plot(t,altitude_data,color='red')
    plt.title('Altitude')  
    plt.xlabel('flying_time')  
    plt.ylabel('height')
    plt.subplot(223)
    plt.plot(t,airspeed_data,color='red')
    plt.title('Airspeed')  
    plt.xlabel('flying_time')  
    plt.ylabel('speed')
    plt.subplot(224)
    plt.plot(t,voltage_data,color='red')
    plt.title('Voltage')  
    plt.xlabel('flying_time')  
    plt.ylabel('Voltage')
    plt.pause(0.1)
...
def print_attributes():
    print "********************************\n"
    print "All the attributes are listed below:\n"
